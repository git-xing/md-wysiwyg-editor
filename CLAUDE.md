...
- **语法版本**：可放心使用较新的 JS/CSS 语法（如原生 CSS 嵌套、`:has()`、可选链、顶层 await 等），无需为旧浏览器/旧运行时做兼容降级。运行环境只有 Electron（VSCode）和 Node 18，且 esbuild（`target: es2020`）会在构建期按需转译，运行时产物始终安全。优先用嵌套语法等现代写法保持代码简洁
