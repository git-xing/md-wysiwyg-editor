...
import { CellSelection, deleteRow, deleteColumn, setCellAttr, TableMap } from '@milkdown/prose/tables';
import type { Editor } from '@milkdown/core';
import type { EditorView } from '@milkdown/prose/view';
import { TextSelection } from '@milkdown/prose/state';
import {
  IconBold, IconItalic, IconStrikethrough, IconCode,
  IconChevronDown, IconSendChat,
  IconAlignLeft, IconAlignCenter, IconAlignRight, IconTrash2,
} from './icons';
...
function isInTableCell($pos: { depth: number; node(d: number): { type: { name: string } } }): boolean {
...
function applyInlineCodeToSelection(view: EditorView, getEditor: GetEditor): void {
  const { state } = view;
  ...
  if (!codeMarkType) {
    console.warn('[selectionToolbar] code mark type not found in schema, marks:', Object.keys(state.schema.marks));
    callCmd(getEditor, toggleInlineCodeCommand);
    return;
  }
  ...
  sel.forEachCell((node: PMNode) => {
    node.descendants((n: PMNode) => {
      if (n.isText && codeMarkType.isInSet(n.marks)) { hasCode = true; }
    });
  });
...
function sBtn(icon: string, title: string, onClick: () => void): HTMLButtonElement {
...
function isAllRowsSelected(sel: CellSelection): boolean {
  if (!sel.isRowSelection()) { return false; }
  const $anchor = sel.$anchorCell;
...
function isAllColsSelected(sel: CellSelection): boolean {
  if (!sel.isColSelection()) { return false; }
  const $anchor = sel.$anchorCell;
  const $head   = sel.$headCell;
  for (let d = $anchor.depth; d >= 0; d--) {
    if ($anchor.node(d).type.name === 'table') {
...
function getBlockContainerText($pos: ResolvedPos): string {
...
function findLineInOriginalSource(source: string, blockText: string): number {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (normalizeForSearch(lines[i]).includes(normalizedBlock)) return i + 1;
  }
  return -1;
}
...
export function setupSelectionToolbar(
  ...
  document.addEventListener('mousedown', (e) => {
    ...
    if (target.closest?.('.milkdown')) { isDragging = true; }
  }, true);

  document.addEventListener('mouseup', () => {
    if (!isDragging) { return; }
    isDragging = false;
    if (lastView) { showAndPosition(lastView); }
  }, true);
  ...
  // ── 内联格式按钮（文字 + 表格模式都显示）──────
  const boldBtn   = sBtn(IconBold, t('Bold') + ' ' + kbd('Mod-b'), () => callCmd(getEditor, toggleStrongCommand));
  const italicBtn = sBtn(IconItalic, t('Italic') + ' ' + kbd('Mod-i'), () => callCmd(getEditor, toggleEmphasisCommand));
  const strikeBtn = sBtn(IconStrikethrough, t('Strikethrough') + ' ' + kbd('Mod-Shift-x'), () => callCmd(getEditor, toggleStrikethroughCommand));
  const codeBtn   = sBtn(IconCode, t('Inline Code') + ' ' + kbd('Mod-e'), () => { const v = getView(); if (v) { applyInlineCodeToSelection(v, getEditor); } });
  ...
  const sendBtn = sBtn(IconSendChat, t('Send to Claude'), () => {
    ...
    let text = view.state.doc.textBetween(selection.from, selection.to, '\n');
    ...
    if (!text.trim() && selection instanceof CellSelection) {
      ...
      for (let d = $anchor.depth; d >= 0; d--) {
        if ($anchor.node(d).type.name === 'table') {
          ...
          text = view.state.doc.textBetween(tableStart + 1, tableEnd - 1, '\n', '\t');
          break;
        }
      }
    }
    ...
    for (let d = $anchor.depth; d >= 0; d--) {
      if ($anchor.node(d).type.name === 'table') {
        ...
        for (let col = 0; col < map.width; col++) {
          ...
          if (cellNode) { ranges.push({ from: cellPos + 1, to: cellPos + 1 + cellNode.content.size }); }
        }
  ...
  function showAndPosition(view: EditorView): void {
    ...
    if (selection instanceof CellSelection) {
      ...
      // 对齐：整列选中（且非整表格）时显示
      const isEntireTable = isEntireTableSelected(selection as CellSelection);
      tableSep.style.display = 'none';
      ...
      clearHeaderBtn.style.display = (isRow && headerRow && !isEntireTable) ? '' : 'none';
      deleteRowBtn.style.display   = (isRow && !headerRow && !isEntireTable) ? '' : 'none';
      deleteColBtn.style.display   = (isCol && !isEntireTable) ? '' : 'none';
      deleteSep.style.display = (isEntireTable || isRow || isCol) ? '' : 'none';
    ...
    for (let d = $from.depth; d >= 0; d--) {
      if ($from.node(d).type.name === 'code_block') { hideToolbar(); return; }
    }
    ...
    if (!inTable) {
      ...
      if (labelEl) { labelEl.textContent = formats[activeLevel]?.[1] ?? 'P'; }
      fmtItems.forEach((item, i) => {
        item.classList.toggle('sel-tb-fmt-item--active', i === 0 ? activeLevel === 0 : i === activeLevel);
      });
    }
