...
import type { Decoration, DecorationSource, EditorView } from '@milkdown/prose/view';

type ViewMutationRecord = MutationRecord | { type: 'selection'; target: Node };
import { IconCopy, IconCheck, IconChevronDown } from './icons';
...
function updateLineNumbers(gutter: HTMLElement, text: string): void {
  ...
  Array.from(gutter.children).forEach((el, i) => { (el as HTMLElement).textContent = String(i + 1); });
}
...
function createLangPicker(
  ...
  function renderList(filter = ''): void {
    ...
    const filtered = LANGUAGES.filter(([val, label]) =>
  ...
  function setActiveIdx(idx: number): void {
    ...
    items.forEach((el, i) => el.classList.toggle('lang-picker-item--focused', i === idx));
  ...
  function outsideClickHandler(e: MouseEvent): void {
    if (!wrapper.contains(e.target as Node) && !dropdown.contains(e.target as Node)) { close(); }
  }
...
export function createCodeBlockView(
...
): { dom: HTMLElement; contentDOM: HTMLElement; update: (n: PMNode) => boolean; ignoreMutation: (m: ViewMutationRecord) => boolean; destroy: () => void } {
  ...
  return {
    ...
    ignoreMutation(mutation: ViewMutationRecord): boolean {
      ...
      return !codeEl.contains(mutation.target as Node) && mutation.target !== codeEl;
    },

    destroy(): void {
      console.log(`[NodeView ${_id}] destroy() 调用`, new Error().stack?.split('\n')[2]?.trim());
      picker.destroy();
      if (copyRestoreTimer) { clearTimeout(copyRestoreTimer); }
    },
  };
