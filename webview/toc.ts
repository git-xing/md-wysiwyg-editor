...
export function initToc(getEditorView: () => EditorView | null): {
  ...
  function refresh(): void {
    ...
    headings.forEach(({ level, text, pos }) => {
      ...
      applyTooltip(item, text, { placement: 'above', truncatedOnly: true });
  ...
  function toggle(): void {
    if (isOpen) { close(); } else { openPanel(false); }
  }
  ...
  function updatePanelPosition(): void {
    ...
    // tab 垂直居中于面板
    const tabTop = topbarBottom + (window.innerHeight - topbarBottom) / 2 - 24;
